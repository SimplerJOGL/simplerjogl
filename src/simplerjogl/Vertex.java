...
public class Vertex
{
	...
	/**
	 * Set to a specific position in 2D Cartesian coordinates in the z=0
	 * plane
	 */
	...
	/**
	 * Copy constructor, duplicates the coordinates of another Vector into
	 * this object
	 */
	...
	/**
	 * @return true if (x, y, z, w) coordinates of both Vectors are
	...
	{
		double[] oldPos = setPosition ((double) x, (double) y, (double) z, (double) w);
		float[] oldPosf = { (float) oldPos[0], (float) oldPos[1], (float) oldPos[2], (float) oldPos[3] };
		return oldPosf;
	}
	...
	/**
	 * For portability, {@literal w<0} is not recommended, w coordinate is
	 * set to absolute value of w parameter
	 * 
	...
	{
		double[] xyzw = getXYZW ();
		float[] xyzwf = { (float) xyzw[0], (float) xyzw[1], (float) xyzw[2], (float) xyzw[3] };
		return xyzwf;
	}

	/**
	 * @return double vector of (x, y, z) coordinates, normalizing
	 *         homogeneous coordinates to (x/w, y/w/, z/w) if w > 0
	 */
	...
	/**
	 * @return float vector of (x, y, z) coordinates, normalizing
	 *         homogeneous coordinates to (x/w, y/w/, z/w) if w > 0
	 */
	...
	public static Vertex normalVector (Vertex A, Vertex B, Vertex C)
	{
		...
		return new Vertex (v1.y * v2.z - v1.z * v2.y, v1.x * v2.z - v1.z * v2.x, v1.x * v2.y - v1.y * v2.x);
	}
}